...
@app.route('/')
def home():
    return render_template('main.html', name='Ramicho')
...
def greet(): 
   if 'user' in request.args: 
        ...
        return render_template('greet.html', user=usr) #http://localhost:5000/greet?user=Ramicho
   else:
...
#     app.run(debug=True) #lokalde çalışmak için
      app.run(host='0.0.0.0', port=80) # EC2 de çalışmak için